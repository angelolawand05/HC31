function [tUs, lfp, fsRaw] = load_csc_interval(cscPath, startUs, endUs)
    tUs = [];
    lfp = [];
    fsRaw = NaN;
    ...
    try
        ...
        warning('Interval read failed; trying full-file read and trimming. This may be slow.');
        try
            [ts, fs, nValid, samples, header] = Nlx2MatCSC(cscPath, [1 0 1 1 1], 0, 1, []);
        catch
            return;
        end
    end
