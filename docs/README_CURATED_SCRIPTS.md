...
These gaps are documented in `docs/REPRODUCIBILITY.md`. The repository also includes compact manuscript-facing values in `derived_data/` so readers can inspect the frozen reported results without rerunning the full raw-data pipeline.
